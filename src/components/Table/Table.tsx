...
interface TableProps {
    ...
    handleDraft?: (id: number) => void;
}


const CustomTable: React.FC<TableProps> = ({  data, handleDelete, handleDraft, handleEdit}) => {
  
  function formatDate(dateString: string):string {
    const [, day, month, year, time] = dateString.match(/(\d+)-(\d+)-(\d+)\s+T(\d+:\d+:\d+)/) || [];
    const [hours, minutes, seconds] = time.split(':').map(Number);
    const monthIndex = Number(month) - 1;
    const dateObj = new Date(Number(year), monthIndex, Number(day), hours, minutes, seconds);
    const options: Intl.DateTimeFormatOptions = { day: 'numeric', month: 'long', year: 'numeric' };
    return dateObj.toLocaleDateString('en-US', options);
  }
  console.log(formatDate('11-05-2023 T23:48:12'));
  
    return (
      <div className="relative overflow-x-auto sm:rounded-lg">
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead className="text-[16px] text-neutral-80 bg-white">
                <tr>
                ...
                data?.map((item:any)=>{
                  ...
                  <tr key={item.id} className="bg-white">
                      ...
                      <td className="flex space-x-5">
                        ...
                        <div className="cursor-pointer" onClick={() => handleDraft && handleDraft(item.id_news)}>
                        ...
                        <div className="cursor-pointer" onClick={() => handleEdit && handleEdit(item.id_news)}>
