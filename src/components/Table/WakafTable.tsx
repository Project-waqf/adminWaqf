...
const WakafTable: React.FC<TableProps> = ({ dashboard, archives, draft, data, handleDelete, handleArchive, handleEdit, handleSort, isSort}) => {
    ...
    const convertToRupiah = (input: any, withDashPostfix = true) => {
    ...
    return (
        ...
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            <thead className="text-[14px] text-text03 bg-white">
                <tr className=''>
                    <th scope="col" className="px-2 text-center py-3 w-30 flex font-normal">
                        Tanggal
                    ...
                    <th scope="col" className={`px-2 py-3 font-normal ${draft || archives ? 'lg:w-[550px] xl:w-[780px]' : 'lg:w-[347px] xl:w-[400px]'}`}>
                        Judul
                    </th>
                    <th scope="col" className={draft || archives ? "hidden" : "px-2 py-3 w-[140px] font-normal"}>
                        Terkumpul
                    </th>
                    <th scope="col" className="px-2 py-3 w-[140px] font-normal">
                        Target
                    </th>
                    <th scope="col" className="text-center py-3 w-[130px] font-normal">
                        Jumlah Hari
                    </th>
            ...
            <tbody>
                {dashboard ?
                    ...
                    data.map((item: any)=> {
                        ...
                        <tr key={item.id} className="">
                            <td scope="row" className="px-2 py-4 w-32">
                            ...
                            <td className="px-2 py-4 w-[140px] flex justify-center">
                ...
                return(
                    <tr key={item.id} className="bg-white">
                        <td scope="row" className="px-2 py-4 w-32">
                        <Typography color='text03' variant='body3' type='normal' className='w-30'>
                            {item.created_at}
                        </Typography>
                        </td>
                        ...
                        <td className="px-2 py-4 w-[140px]">
                            <Typography color='text03' variant='body3' type='normal' className={`w-[120px] truncate ${item.collected >= item.fund_target ? 'bg-green-500 text-white text-center rounded-[12px] ' : item.collected < item.fund_target && item.due_date == 0 ? 'bg-error-90 text-white text-center rounded-[12px] ' : ''}`} >
